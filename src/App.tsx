  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-300 flex flex-col md:flex-row font-sans selection:bg-cyan-500/30 selection:text-white" id="pos-application-container">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <footer className="border-t border-white/10 bg-zinc-950 px-4 py-2 text-[10px] font-mono text-zinc-400 flex flex-wrap items-center justify-between gap-2 z-30 shrink-0" id="pos-system-footer">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5 text-zinc-300">
              <Calendar className="h-3.5 w-3.5 text-cyan-400 shrink-0" />
              SYSTEM DATE: <strong className="text-cyan-300 font-bold bg-cyan-950/60 border border-cyan-500/20 px-2 py-0.5 rounded ml-1">{systemDate}</strong>
            </span>
            <span className="text-zinc-600 hidden sm:inline">•</span>
            <span className="flex items-center gap-1.5 text-zinc-400 hidden sm:flex">
              <Clock className="h-3.5 w-3.5 text-zinc-500 shrink-0" />
              SYS TIME: <strong className="text-zinc-200 font-bold">{systemTime.toLocaleTimeString()}</strong>
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={syncWithRealClock}
              className={`px-2 py-0.5 rounded border text-[9px] font-mono uppercase font-bold flex items-center gap-1 transition ${
                isRealTodaySynced 
                  ? 'bg-emerald-950/40 text-emerald-400 border-emerald-500/20' 
                  : 'bg-amber-950/60 text-amber-300 border-amber-500/30 hover:bg-amber-900/80'
              }`}
            ...
            >
              <RefreshCw className={`h-2.5 w-2.5 ${isRealTodaySynced ? '' : 'animate-spin'}`} />
              {isRealTodaySynced ? 'REALTIME SYNCED' : 'SYNC TO TODAY'}
            </button>
            <span className="text-[9px] text-zinc-500 uppercase tracking-widest hidden md:inline">PALE ORE POS v2.6</span>
          </div>
        </footer>
      </div>
